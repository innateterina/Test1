...
while True:
    input_name = input('Как тебя зовут? ')
    input_age = input('Сколько тебе лет? ')

    if not input_age.isdigit() or int(input_age) <= 0:
        print('Ошибка, повторите ввод')

    elif 0 < int(input_age) < 10:
        print('Привет, шкет', input_name)

        answer = input('Желаете выйти? (Д/Y)')
        if answer.upper() in ('Y', 'Д'):
            break

    elif input_age == 10 or int(input_age) <= 18:
        print('Как жизнь', input_name, '?')

        answer = input('Желаете выйти? (Д/Y)')
        if answer.upper() in ('Y', 'Д'):
            break

    elif 100 > int(input_age) > 18:
        print('Что желаете, ', input_name, '?')
        ...
        print(input_name, ', вы лжете - в наше время столько не живут...')

        answer = input('Желаете выйти? (Д/Y)')
        if answer.upper() in ('Y', 'Д', 'y', 'д'):
            break
